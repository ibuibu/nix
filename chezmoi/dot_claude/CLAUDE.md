...
コードコメントには非自明な WHY だけを書く。書くのは隠れた制約・workaround を入れた理由・読み手が驚く挙動といった、コードから復元できない情報に限る。

書かないもの:

- WHAT（コードを読めば分かること。`// ユーザー ID を取得する` の類）
- 変更履歴（「〜を追加した」「旧実装では〜だった」等）
- タスク ID 参照（`(ABC-1234)` 等）
...
- PRコメントでは、直訳調・比喩的な独自表現を避け、素直で平易な日本語を使う。
  - 悪い例:「静かな退行」「漏洩固定テスト」「兄弟（sibling）」
  - 良い例:「気づきにくいバグ」「〜を確認するテスト」「device_config（具体名で書く）」
- PRレビューコメントは**ですます調**で書く。あわせて次を守る。
  - **冒頭1文で用件を書く**（何が問題か / 何をしてほしいか）。現状説明から始めて結論が後ろに来る構成にしない。
  - **抽象語より具体名**。「取り込み経路」ではなく「デバイスからのデータ受信処理（`time_series` の `save`）」のように、関数名やモジュール名を添える。
  - **括弧の入れ子を解体する**。修飾が重なったら文を分ける。
  - 抽象的な指摘には**具体例を1つ**添える。
  - 本題と関係の薄い補足は「なお」で末尾に分離する。
  - **太字は使わない**。ですます調は語尾で強弱がつくので装飾はノイズになる。
  - 命令・断定を避け、提案として書く。「〜してください」ではなく「〜するとよさそうです」「〜な気がしました」のように書く。
